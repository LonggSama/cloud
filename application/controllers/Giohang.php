...
class Giohang extends CI_Controller {
	...
	public function index(){
        $this->data['title']='AppPhakeStore.vn - Your cart';
    ...
    public function checkout(){
        if($this->session->userdata('session24hStore')){
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $this->data['title']='AppPhakeStore.vn - Order';
    ...
    public function info_order(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            $config['smtp_user']    = 'sale.appphakestore@gmail.com';
            ...
            $this->email->from('apphakestore@gmail.com', 'AppPhakeStore');
            $list = array('longnvhbhaf190218@fpt.edu.vn');//Mặc định là email của admin
            $this->email->to($list);
            $this->email->subject('AppPhakeStore system');
            $this->email->message('Your website has just received a new order, login to the admin page to view details!');
            if ($this->email->send()) {
            ...
            $this->data['title']='AppPhakeStore.vn - Information line';
    ...
    public function thankyou(){
        if($this->session->userdata('info-customer')||$this->session->userdata('session24hStore')){
            ...
            $config['smtp_port']    = '465';
            $config['smtp_timeout'] = '7';
            $config['smtp_user']    = 'sale.apphakestore@gmail.com';
            $config['smtp_pass']    = 'jgqunljqbtoiervp';
            $config['charset']    = 'utf-8';
            $config['newline']    = "\r\n";
            $config['wordwrap'] = TRUE;
        ...
        $this->data['title']='AppPhakeStore.vn - Order results';
    ...
    public function district(){
        ...
        $html="<option value =''>--- Choose your district ---</option>";
        foreach ($list as $row) 
        {
