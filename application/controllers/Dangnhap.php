...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            if($this->Mcustomer->customer_login($email, $password)!=FALSE){
                ...
                $this->data['title']='Sign in to your account';
                $this->data['view']='dangnhap';
                $this->load->view('frontend/layout',$this->data);
            }
        }else{
            $this->data['title']='AppPhakeStore - Login account';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->email->from('appphekestore@gmail.com', 'AppPhakeStore');
            $list = array('longnvhbhaf190218@fpt.edu.vn');
            $this->email->to($list);
            $this->email->subject('System AppPhakeStore');
            $this->email->message('User '.$this->input->post('username').' just become a member on your website!');
            if ($this->email->send()) {
                $this->data['msg']='Sign Up Success !';
                redirect('dang-nhap','refresh');
            } 
        }  
        $this->data['title']='AppPhakeStore - Register account';   
    ...
    function check_username(){
        ...
        if($this->Mcustomer->customer_check_username($username)){
            $this->form_validation->set_message(__FUNCTION__, 'Username is already taken');
            return FALSE;
        }
        return TRUE;
    }
    ...
    function check_mail(){
        ...
        {
            $this->form_validation->set_message(__FUNCTION__, 'Email is already in use');
            return FALSE;
        }
        return TRUE;
    }
